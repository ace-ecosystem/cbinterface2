...
browsing = pd.read_sql_query("SELECT datetime(((visits.visit_time/1000000)-11644473600), 'unixepoch') AS date, urls.url, urls.title FROM urls, visits WHERE urls.id = visits.url;", db)
...
downloads = None
try:
    downloads = pd.read_sql_query("SELECT datetime(downloads.start_time, 'unixepoch') AS date, downloads.url, downloads.full_path, downloads.received_bytes, downloads.total_bytes FROM downloads;", db)
except:
    downloads = pd.read_sql_query("SELECT datetime(((downloads.start_time/1000000)-11644473600), 'unixepoch') AS date, downloads.target_path, downloads_url_chains.url, downloads.received_bytes, downloads.total_bytes \
        FROM downloads, downloads_url_chains WHERE downloads.id = downloads_url_chains.id;", db)
